...
# Sweeps the DDTW colocation baseline over the recording length x sampling rate
# x rolling window grid and appends one row per dataset to a results CSV.
#
...
#
# Reads the filter_abs datasets built by
# scripts/dataset_creation/coloc_baseline_datasets.sh.  The "first N seconds" of
# a trainride is the dataset's trainride_start_seconds; the chunk duration is
# held fixed at the smallest one, since the evaluation stitches the chunks back
# together and compares the whole FIRST-second trace.

# Paths
DATA_BASE_DIR="${COLOC_DATA_BASE_DIR:-datasets/coloc_baseline}"
...
if [[ ! -d "${DATA_BASE_DIR}" ]]; then
  ...
  echo "Generate it first with scripts/dataset_creation/coloc_baseline_datasets.sh"
  exit 1
fi
  ...
  for first in "${FIRST_SECONDS[@]}"; do
    for sampling_rate in "${SAMPLING_RATES[@]}"; do
      for rolling_window in "${ROLLING_WINDOWS[@]}"; do
        ...
        log_path="${LOG_DIR}/ddtw_K${K}_${dataset_name}.log"
        n_run=$(( n_run + 1 ))
        echo
        echo "=== [${n_run}/${n_runs_total}] ddtw ${train_type} first=${first}s ${sampling_rate}Hz window=${rolling_window} === (log: ${log_path})"

        status=0
        apptainer exec --bind "${REPO_ROOT}:${REPO_ROOT}" --pwd "${REPO_ROOT}" "${PYTHON_SIF}" \
          evaluate-coloc-distance ddtw "${pkl_path}" "${results_csv}" \
          --K "${K}" \
          "${EXTRA_ARGS[@]}" \
          --chunksize "${CHUNKSIZE}" \
          --test-frac "${TEST_FRAC}" \
...
if (( ${#MISSING[@]} > 0 )); then
  echo "Missing datasets (${#MISSING[@]}), create them with scripts/dataset_creation/coloc_baseline_datasets.sh:"
  printf '  %s\n' "${MISSING[@]}"
fi
