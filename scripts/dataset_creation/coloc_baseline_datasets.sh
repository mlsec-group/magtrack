...
OUTPUT_BASE_DIR="datasets/coloc_baseline"

# Extraction: absolute field magnitude only, no notch/highpass filtering.
EXTRACTION_FUNCTION_NAME="filter_abs"
...
TRACE_CACHE_DIR="${COLOC_BASELINE_TRACE_CACHE_DIR:-${OUTPUT_BASE_DIR}/trace_cache}"
...
TRAINRIDE_START_SECONDS=(0 10 30 60 300 600 900)

# Datasets to emit, as "name:train_types:output_dir".
...
for rolling_window in "${ROLLING_WINDOWS[@]}"; do
  for duration in "${DURATIONS[@]}"; do
    for sampling_rate in "${SAMPLING_RATES[@]}"; do
      ...
      echo "create-colocation-datasets --dataset-path \"${TRAINTRACK_PATH}\" --output-path \"${OUTPUT_BASE_DIR}\" --duration \"${duration}\" --sampling-rate \"${sampling_rate}\" --rolling-window \"${rolling_window}\" ${START_ARGS[*]} ${SPEC_ARGS[*]} --extraction-function-name \"${EXTRACTION_FUNCTION_NAME}\" --normalize trace ${CACHE_ARG} >> \"${log_path}\" 2>&1" >> "${COMMANDS_FILE}"
    done
  done
done
...
echo "Extraction function:   ${EXTRACTION_FUNCTION_NAME}"
...
if [[ -n "${TRACE_CACHE_DIR}" ]]; then
  ...
  apptainer exec --bind "${REPO_ROOT}:${REPO_ROOT}" --pwd "${REPO_ROOT}" "${PYTHON_SIF}" \
    ...
    --extraction-function-name "${EXTRACTION_FUNCTION_NAME}" \
