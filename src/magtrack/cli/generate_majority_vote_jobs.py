...
app = typer.Typer(help="Generate GNU parallel job for majority vote evaluation.")
...
def _majority_cmd_args(
    ...
    """Return the list of majority vote flag fragments (no dataset)."""
    return [
...
def _build_parallel_script(
    ...
    if module_mount_path:
        ...
        module_mount_path = ""
    majority_args = " ".join(_majority_cmd_args(
        model_path=model_path, hparams_path=hparams_path,
        test_fraction=test_fraction, threshold=threshold, batch_size=batch_size,
        seed=seed, n_jobs=n_jobs, results_dir=results_dir,
    ))

    script = f"""#!/usr/bin/env bash
set -euo pipefail

# Self-contained GNU-parallel runner for majority vote evaluation
...
COMMANDS_FILE="${{OUTPUT_DIR}}/majority_commands.txt"
PARALLEL_JOBLOG="${{OUTPUT_DIR}}/majority_joblog.tsv"
...
for dataset_path in "${{DATASETS[@]}}"; do
  ...
  cmd="env CUDA_VISIBLE_DEVICES={gpu_device} apptainer exec --nv --bind \"${{BIND_PATH}}\"{module_mount_path} \"${{PYTHON_SIF}}\" python src/magtrack/cli/evaluate_majority_ml.py main --dataset-path \"${{dataset_path}}\" ${{MAJORITY_ARGS}}"
...
def parallel(
        ...
        results_dir: Path = typer.Option(Path("results/coloc_ml/majority_vote"),
                                         help="Directory where evaluation CSV files are saved."),
        ...
        parallel_output_dir: Path = typer.Option(
            Path("results/coloc_ml/majority_logs"),
    ...
    """Generate a self-contained GNU-parallel bash script for majority vote evaluation."""
